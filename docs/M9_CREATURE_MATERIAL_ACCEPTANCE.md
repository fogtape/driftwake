...
- 用专用 Image 2 high 牙釉 PBR 取代鲨鱼牙齿错误复用的帆布材质；在真实口缘内建立代码原生 oral rig v1：上颌 13、下颌 11 颗的主/次两层牙列、两条牙龈带、开口内腔和独立下颌 pivot；两块鲨齿战利品板继续复用同一牙釉材质；
...
| 深潮鲨牙釉 | `prepare_imagegen_material.py`：1024、seam 144、normal 0.22、roughness 116-176、boundary 优化 | x=`5.61/0.88x`，y=`4.77/1.00x`，boundary=`(1,1)` | B 通过 2x2；写入共享图集，不新增运行纹理；24 齿由两个共用一个 LatheGeometry 的实例批次组成 |
...
- 旧车削体鼻端由未封口尖端改为封闭钝吻前盘；`shark-mouth-rim` 位于真实前表面，`shark-mouth-cavity` 为开口圆柱内腔，`shark-mouth-lining` 后移形成可辨深度，不再把口腔退回成贴在躯干前的平面；
- 双眼使用 24 段圆面、半径 0.09，并由严格侧向旋转为嵌入钝吻的侧前向；测试锁定局部法线朝前，正面扑咬不再只显示一条边或外凸圆盘；采集伤痕和鲨肉只使用 `sharkFlesh`；
- `shark-oral-rig` 以 `shark-upper-jaw` 和 `shark-lower-jaw-pivot` 分层；24 颗 `LatheGeometry` 牙齿分为上颌 13 / 下颌 11 的主/次两层，两个 `InstancedMesh` 共用同一牙形几何，`shark-gingiva-upper` / `shark-gingiva-lower` 为可替换的牙龈带，所有牙齿和两块鲨齿战利品板都绑定 `sharkTooth` / `graywake-tooth-enamel`；
- `sharkGum` 是单独的 atlas PBR 材质，保持 TEX-050 原始来源可审计并以不同 roughness、normal 与克制 emissive 区分牙龈；专用牙龈位图尚未获 provider 输出，不能宣称已完成新的 Image 2 贴图；
- `SharkSystem` 按巡游、接近、蓄势、冲击、回摆、浮尸阶段连续推进 `jawOpen`，驱动下颌旋转和小幅位移；`DriftwakeGame` 公开 `data-shark-jaw-open`，单元测试锁定峰值与 `[0,1]` 范围；
...
专用材质场景走真实水中追击 AI；页面观察器只在真实 `windup + counterWindow=true` 的同一微任务原子读取玩家当前 framebuffer，然后才触发正常失焦冻结。renderer 计数、四类 atlas 区域、`teeth=24`、`jawOpen>=0.22`、眼/牙焦点和 framebuffer 内容必须有效；水下场景因礁区与水体资源允许最多 36 张纹理。独立 `shark-combat visual` 仍走正式咬筏路径并把实际游戏预算锁定为 `<=32`。取证过程不移动相机、不二次 render，也不改变 AI、玩法状态或材质。

- 水中材质帧（采用 F、TEX-052 与 oral rig v1）：`34 textures / 118 geometries / 47 calls / 99,830 triangles`，`teeth=24`、`jawOpen=0.512`，最近眼/牙点积 `0.962/0.977`；614x384 活动 framebuffer 为 `variation=253 / nonBlack=233,682`，已人工复核钝吻、嵌入式双眼、开口内腔、上下两层象牙釉牙列、下颌、采用 F 鲨皮、木矛、筏底与礁床同屏；
- 正式咬筏帧（采用 F、TEX-052 与 oral rig v1）：`windup progress=0.243`、`0.727s` 到冲击、一次预兆/零结算，反击卡和准星均存在；`teeth=24`、`jawOpen=0.439`、四类 atlas 区域齐全，`32/32` 纹理，脸/眼/牙点积为 `0.942/0.977/0.931`；
...
- 4x4 图集跨系统回归：水下为 `29 / 118 / 256 / 140,650`、21 个 PBR 槽与 Context/模拟健康；本机原始 readback 为零时，脚本只在 `inspectCanvasPixels` 已验证 CDP 合成帧后明确写入“composited fallback”，不冒充 raw framebuffer。周界为 `30 / 93 / 129 / 105,592`、导航合金/紧固件六槽和同次求值原始 framebuffer `variation=202 / nonBlack=2,869` 通过。结构材质帧为 `30 / 79 / 110 / 113,924` 且 Context/模拟健康；本机软件 WebGL 在第二次连续锤击的既有 8 秒输入时序等待超时，不能把该单次回归重跑写作三锤发布级通过。
