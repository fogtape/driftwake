...
> 日期：2026-07-23
> 版本：`0.22.5`
> 状态：`APPROVED`（Image 2 来源、PBR、模型朝向、共享图集、正式玩法预算与软件 framebuffer 闭环；目标真实 GPU 仍属发布门禁）
...
- 将既有鲨皮 roughness 精确打包进 albedo alpha，减少一张运行纹理；这只是无损物理重打包，不宣称历史程序鲨皮已完成创意升级；
...
14 套独立审计 PBR 以 4x4 写入 `saltcrown-shared-pbr-atlas.webp` 和对应 normal atlas：4096x4096，每格 1024、核心 960、四周 32 像素周期 gutter。albedo RGB 与 roughness A 共图，normal 独立；保存后 alpha 逐像素一致。两格保留为空。相比临时 3840/896 核心方案，正式版恢复既有 960 核心，不以纹理预算为由降低历史材质分辨率。

鲨皮继续使用 1024 normal；`shark-skin-packed.webp` 将既有 albedo RGB 与既有 roughness A 精确合并，shader 读取 alpha。旧三图保留审计来源，但运行时少加载一张 roughness。下一轮仍要按质量审计判断 TEX-003 程序源是否需要 Image 2 high 创意替换。
...
- `sharkMaterialMaps` 必须同时报告 `[graywake-mouth-lining]`、`[graywake-shark-flesh]`、`[graywake-lateral-eye]`、共享双图集和 packed 鲨皮两图；
...
专用材质场景走真实水中追击 AI；页面观察器只在真实 `windup + counterWindow=true` 的同一微任务原子读取玩家当前 framebuffer，然后才触发正常失焦冻结。四项 renderer 计数、三类 atlas 区域和 framebuffer 内容必须有效；水下场景因礁区与水体资源允许最多 36 张纹理。独立 `shark-combat visual` 仍走正式咬筏路径并把实际游戏预算锁定为 `<=32`。取证过程不移动相机、不二次 render，也不改变 AI、玩法状态或材质。

- 水中材质帧：`34 textures / 114 geometries / 42 calls / 96,246 triangles`，最近眼片点积 `0.9704`；614x384 活动 framebuffer 为 `variation=253 / nonBlack=232,880`，已人工复核钝吻、嵌入式双眼、浅口腔、木矛、筏底与礁床同屏；
- 正式咬筏帧：`windup progress=0.243`、`0.727s` 到冲击、一次预兆/零结算，反击卡和准星均存在；三类 atlas 区域齐全，`32/32` 纹理，脸/眼点积为 `0.934/0.972`；
- `shark-combat-windup` 分布采样为 `variation=232 / nonBlack=2,880`。本机 CDP 合成超时后的 DOM 兜底不作为美术图，水中活动 framebuffer 才是本轮视觉证据。
- 水中战利品逻辑档使用 320x200/低渲染负载，但仍运行正式固定步、AI、输入、存档和完整 4096 图集：一次真实刺击后连续完成四段按住割取，结算 `3 鲨肉 / 1 鲨皮 / 2 鲨齿`、矛耐久 `90 -> 89`、零世界掉包，结束时 `34 textures / 123 geometries / 51 calls / 67,146 triangles` 且 Context 健康；
- 4x4 图集跨系统回归：水下为 `29 / 118 / 255 / 140,618`、21 个 PBR 槽与 framebuffer 有效；结构为 `30 / 78 / 110 / 113,924`、断面/紧固件槽与三锤修复通过；周界为 `30 / 89 / 129 / 105,592`、导航合金/紧固件六槽与合成帧通过。
...
- 目标真实 GPU 的 1280x720/30、1920x1080/60 双画质、夜间/风暴/水下眼口材质响应与 20 分钟长稳；
- 木矛和潮鸣震叉近景中的眼片、口缘、鳃衬、胸鳍遮挡和伤痕 mip/各向异性；
- 鲨鱼预兆、扑空、反击、受击、浮尸和采集声音在真实音频设备上的混音；
- 最终 DCC 鲨体 UV、眼球体积、口腔/牙齿层、鳃盖变形、受击和浮尸动画；
- TEX-003 历史程序鲨皮的 Image 2 high 候选对比，以及源图授权、C2PA、商标和商业作品相似性复核。
