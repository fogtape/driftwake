...
> 日期：2026-07-23
> 版本：`0.22.6`
> 状态：`APPROVED`（Image 2 来源、PBR、模型朝向、共享图集、正式玩法预算与软件 framebuffer 闭环；目标真实 GPU 与最终 DCC 鲨齿仍属发布门禁）
...
- `sharkMaterialMaps` 必须同时报告 `[graywake-mouth-lining]`、`[graywake-shark-flesh]`、`[graywake-lateral-eye]`、共享双图集和采用 F 的 packed 鲨皮两图；`Materials.test.ts` 同时锁定鲨皮 `map / normalMap / roughnessMap` 的三槽绑定和 alpha shader 标记；
...
- 水中材质帧（采用 F）：`34 textures / 114 geometries / 42 calls / 96,246 triangles`，最近眼片点积 `0.9659`；614x384 活动 framebuffer 为 `variation=253 / nonBlack=232,404`，已人工复核钝吻、嵌入式双眼、浅口腔、采用 F 鲨皮、木矛、筏底与礁床同屏；
- 正式咬筏帧（采用 F）：`windup progress=0.226`、`0.743s` 到冲击、一次预兆/零结算，反击卡和准星均存在；三类 atlas 区域齐全，`32/32` 纹理，脸/眼点积为 `0.902/0.951`；
- `shark-combat-windup` 分布采样为 `variation=231 / nonBlack=2,880`。本机 CDP 合成超时后的 DOM 兜底不作为美术图，水中活动 framebuffer 才是本轮视觉证据。
- 水中战利品逻辑档使用 320x200/低渲染负载，但仍运行正式固定步、AI、输入、存档和完整 4096 图集：一次真实刺击后连续完成四段按住割取，结算 `3 鲨肉 / 1 鲨皮 / 2 鲨齿`、矛耐久 `90 -> 89`、零世界掉包，结束时 `34 textures / 121 geometries / 48 calls / 66,558 triangles` 且 Context 健康；
...
- 最终 DCC 鲨体 UV、眼球体积、口腔/牙齿层、鳃盖变形、受击和浮尸动画；牙齿尚未达到专用 DCC 层的屏幕覆盖与体积验证门槛；
