...
- 运行时 albedo atlas 为 4096x2048 RGBA，RGB 保存 albedo、A 精确保存 roughness；
- normal atlas 为独立 4096x2048 RGB；4x2 的最后一格为中性保留区；
...
- atlas 生成后逐像素验证保存 alpha 与源 roughness atlas 完全一致；最终运行时只增加两张纹理，保持 `32/32`。

布局、gutter、核心尺寸和七组 UV offset/scale 记录在 `artifacts/imagegen/underwater-pbr/atlas-layout.json`。
...
- `Materials.test.ts` 锁定加载路径、七个区域、两图复用、alpha roughness 与三套 UV 注入；运行时 dataset 公开 21 个带区域名的语义槽用于浏览器硬门禁。
...
- `renderer.info.memory.textures=32`、geometries=`118`、draw calls=`256`、triangles=`140,650`；
- `contextHealthy=true`、`simulationActive=true`；
- 首轮绑定帧直接画布采样 `variation=127/nonBlack=2880`；最终海草提亮复核时本机 GLES 直接读回偶发返回零帧，脚本改用 `1,137,972` 字节浏览器合成帧确认非空，并继续导出 864x540 WebGL framebuffer；
