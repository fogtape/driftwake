...
> 更新日期：2026-07-25
> 状态：M9 候选发布门禁已自动核对全部 113 个运行时位图与 45 个采用源图，既有生物/结构/环境 PBR 与无降采样共享图集保持通过；发布前仍需项目自身许可选择、最终 DCC 替换与相似性复核

## 管线原则

- 不从任何商业游戏提取、描摹或重新分发模型、贴图、动画、UI 和音频；
- AI 位图通过项目 `scripts/imagegen` 调用配置 provider 生成，仓库不保存服务 URL 或密钥；
- 单次 `generate` / `edit` 只对 provider 传输断连、超时、临时服务错误和限流执行项目包装器的有界退避；参数、提示词、文件和批量任务错误不重试，不能把失败请求计作采用源图；
...
- `npm run release:check` 枚举全部 `public/assets` 与采用的 `artifacts/imagegen/*-raw.png`；任一文件未在本清单出现、显式路径缺失或生产依赖许可未登记都会拒绝候选包。
