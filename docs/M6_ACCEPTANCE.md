...
> 当前状态：`DONE`（钓鱼、烹饪/净水、天气农业、盐翼鸟害、原创材质与自动浏览器证据闭环完成；目标真实 GPU 十分钟体验仍为发布门禁）
> 本次切片：天气驱动作物、风暴驱鸟、作物/盐翼七套 PBR 与三场景浏览器门禁
> 版本：`0.18.0`
> 日期：2026-07-21
...
- 作物每个固定步直接读取 `NavigationSystem.getWeather()`，不创建第二套天气时钟；平静期维持 1.0 倍生长/水耗，积云强风最高降低 10% 生长并提高 30% 水耗；
- 强风暴以 0.024/s 雨量补水、0.72 倍生长与 0.55 倍蒸腾运行；雨水可催发未浇种子、将临枯作物恢复为生长并清空枯萎计时，但不会以暴雨同时提供满速生长；
- 消散阶段按同一风暴强度平滑降低雨量并恢复生长/水耗，不通过相位跳变制造数值断层；领域测试覆盖大步与 60Hz 固定步一致性；
- 强风暴冻结盐翼来访时钟，并把真实 `circling/diving/feeding` 个体转为 `fleeing`；同帧雨水恢复和驱鸟合并为一条玩家通知，避免互相覆盖；
- `planting-weather` 证据中两盆分别从 `dry/sown` 恢复为 `growing`，恢复次数恰好 2，鸟从 `circling` 退出且驱散次数恰好 1；HUD 显示“雨水补给”，交互提示显示“风暴雨养”。

## 作物与盐翼原创材质

- 盐冠活叶、同源编辑枯叶、潮果、盐翼体羽、飞羽、角质与虹膜均由项目 `scripts/imagegen` 使用 `gpt-image-2`、`quality=high` 生成 2048x2048 原创源图；七张源图归档，运行时为 21 张独立 1024 PBR 图；
- 六套平铺表面接缝绝对值均低于 17，相对内部差均低于 1.17；2x2 人工检查没有硬边/十字带，normal/roughness 与叶脉、果皮和羽片细节相关；虹膜保留非平铺中心布局；
- 角质首版长纤维偏石纹，同源精修仍有大理石感，均拒绝；采用第三版为低对比毫米级角质，不因生成成本降低验收标准；
- 盐翼双眼由无方向纯色小球改为有朝向圆形虹膜面与飞羽眼缘；叶面/羽面只使用低强度贴图驱动散射补偿，仍受环境光控制，避免背光黑片也不成为自发光物；
- `planting-crop-materials-desktop.png` 同屏证明活株、成熟果和枯株；`planting-bird-desktop.png` 证明体羽、飞羽、喙、腿趾和虹膜；所有诊断都要求完整 9/12 项 map 名称且不含 `none`。

## 种植自动与浏览器证据

- 全量 `npm test`：46 个测试文件、293 项通过；相对 0.17.0 新增天气倍率、雨水萌发/救活、固定步一致性、风暴禁鸟和 21 张材质加载/模型绑定覆盖；
- `npm run build` 通过：React 主入口约 350.84KB、3D 世界约 1.062MB、Rapier 约 2.237MB；独立类型、捕获/稳定性脚本、PBR 处理器和 `git diff --check` 通过；
- 三个 1024x640 场景均保持 Context 与模拟健康，设备架/快捷栏/交互提示无越界或二维相交；软件 GLES 直接像素为零时只使用经过尺寸、编码与非空检查的合成帧，不冒充目标 GPU；
- `planting-bird` 在截图后仍真实执行 `E` 驱赶，并通过通知出现与警报隐藏双断言；钩具唯一所有权继续要求手持可见、投射物/绳索隐藏。

复现命令：

```sh
CAPTURE_ONLY=planting-weather DRIFTWAKE_URL=http://127.0.0.1:4173 npm run capture
CAPTURE_ONLY=planting-materials DRIFTWAKE_URL=http://127.0.0.1:4173 npm run capture
CAPTURE_ONLY=planting-bird DRIFTWAKE_URL=http://127.0.0.1:4173 npm run capture
CAPTURE_ONLY=planting DRIFTWAKE_URL=http://127.0.0.1:4173 npm run capture
```

## 外部发布门禁

- 在目标真实 GPU 的 1280x720/30 与 1920x1080/60 下，用真实鼠标连续钓鱼、维护烹饪与种植至少 10 分钟，检查时序、PBR、天气、HRTF/混音和运行时预算；
- 该外部设备门禁不阻挡 M6 代码/自动闭环转为 `DONE`，也不伪写为本机已通过；全仓 `SYS-022` 因其他历史材质整改继续保持 `DOING`。
