...
> 当前状态：`DOING`（三档存档、备份恢复、生命周期保存、无障碍输入、工具/打捞与岛屿/岸上两批历史材质已闭环；其余最终资产、真实 GPU 与发布证据仍在后续切片）
> 当前版本：`0.22.2`
> 日期：2026-07-22
...
- 全流程混音、灯光、其余历史材质回溯和最终 DCC 替换；工具/打捞整改详见 [M9 材质整改验收记录](M9_MATERIAL_ACCEPTANCE.md)，岛屿/岸上整改详见 [M9 岛屿材质验收记录](M9_ISLAND_MATERIAL_ACCEPTANCE.md)；
