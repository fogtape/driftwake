...
> 当前状态：`DOING`（三档存档、备份恢复、生命周期保存、无障碍输入，以及工具/打捞、岛屿/岸上、水下礁区、结构/防御、鲨鱼口腔/眼部/主体皮肤六批历史材质已闭环；最终 DCC 牙齿、真实 GPU 与发布证据仍在后续切片）
> 当前版本：`0.22.6`
> 日期：2026-07-23
...
- 新增深潮鲨口缘/鳃衬、圆瞳侧眼与主体皮肤三套 `gpt-image-2 high 2048x2048` 原创源图；裂瞳以及五个鲨皮候选明确拒绝，完整提示词、采用/拒绝源与 PBR 参数均归档；
- 眼、口、伤痕/鲨肉使用三个独立 atlas 区域；共享图集保持 4096x4096/14 区与 960 核心，采用 F 鲨皮以 direct packed RGB/A + normal 运行，不占用新 atlas 格且正式咬筏仍为 `32/32`；
- 修复鲨体根节点正 Z `lookAt` 与负 Z 鼻端相反造成的尾朝目标问题；巡游、追击、扑咬、退场和浮尸共用正确朝向，眼口在真实袭击中不再被整段躯体遮挡；
- 攻击中心停距按 1.85m 吻部调整为木筏 3.6m、水中 3.85m，反击窗不再贴近裁剪面；采用 F 的水中微材质场景在真实活动帧得到 `34 / 114 / 42 / 96,246`、`variation=253 / nonBlack=232,404` 后才冻结，正式咬筏继续锁定 `32/32`。最终 DCC 鲨齿层仍为单独发布门禁。详细证据见 [M9 生物微材质验收记录](M9_CREATURE_MATERIAL_ACCEPTANCE.md)。
