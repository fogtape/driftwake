...
工具钢和导航合金三图已从运行目录移至 `artifacts/imagegen/shared-pbr-sources/`，旧 4096x2048 水下图集已删除。`0.22.4` 使用新审计目录重建时，两张图集 SHA-256 与迁移前完全一致，证明该次迁移没有重采样或降质：
...
- 结构损坏/修复：鲨鱼真实双咬 `75 -> 41 -> 7`，冷启动恢复后横截面数量为 1，18 个材质槽完整；`0.22.5` 4x4 图集复验为 `30 textures / 78 geometries / 110 calls / 113,924 triangles`。三锤真实修补 `7 -> 51 -> 95 -> 110`，漂木 `12 -> 9`、锤耐久 `80 -> 77`，最终损坏/临界/横截面均归零；
- 周界防御：三块缘甲与载货潮兜保持筏体局部附着，护栏/紧固件六个 PBR 槽分别命中 `navigation-alloy` 和 `stormbrace-fastener-alloy`；`0.22.5` 复验为 `30 textures / 89 geometries / 129 calls / 105,592 triangles`；
- 水下回归：删除旧图集后七个区域的 21 个槽仍全部绑定共享图集，renderer 从 `32` 降为 `29 textures`，其余为 `118 geometries / 255 calls / 140,618 triangles`；`0.22.5` 4x4 图集画布像素为 `variation=126/nonBlack=2880`；
...
结构系统新增“临界横截面出现、修复后归零”测试；材质测试覆盖当前 14 个 atlas 区域的唯一 program cache key、工具钢/导航合金/鲨鱼微材质共享绑定和 roughness alpha shader。目标真实 GPU 的 1280x720/30、1920x1080/60、风暴夜间金属响应、长时间扩建和最终 DCC/LOD 仍保留为 M9 发布门禁。
