...
import type { CameraMotionProfile } from '../domain/settings';
...
export class CameraMotionFilter {
  ...
  update(
    ...
    if (twistLength > ANGLE_EPSILON) {
      this.yawRotation.set(0, raftRotation.y / twistLength, 0, raftRotation.w / twistLength);
    } else {
    ...
    this.targetTilt.identity().slerp(this.raftTilt, profile.raftTiltScale);
    ...
    if (Number.isFinite(deltaSeconds) && deltaSeconds > 0) {
      ...
      if (angle > ANGLE_EPSILON) {
        ...
        const speedAlpha = Math.min(1, (profile.maximumTiltSpeed * deltaSeconds) / angle);
