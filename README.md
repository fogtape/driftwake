...
原创桌面网页 3D 海上生存游戏。当前版本为 `0.19.0` 高质量纵向切片，不以基础 Demo 为完成标准。
...
当前仍不是完整游戏。M6 钓鱼、烹饪/净水、天气农业、盐翼鸟害以及 M8 三座远海目的地/海图/专用材质的代码与自动视觉闭环已经完成；研究成长节奏、无说明玩家验收、目标真实 GPU 鼠标/双画质门禁、更多深水生态资源、潜水装备、最终蒙皮资产和 M9 发布系统仍按 [项目追踪](PROJECT_TRACKER.md) 继续开发。
...
M8 专用截图目标包括 `signal-destinations`、`signal-destination-materials` 和 `signal-chart`；航海图桌面/窄屏证据分别使用合成帧与 DOM 覆盖层捕获，避免暂停 WebGL 背景抢占面板。详见 [M8 验收记录](docs/M8_ACCEPTANCE.md)。
