...
原创桌面网页 3D 海上生存游戏。当前版本为 `0.22.15` 高质量纵向切片，不以基础 Demo 为完成标准。
...
- 新航次使用从背包、设备、饥渴和筏格真值实时推导的情境目标，依次表达打捞净水器材料、部署、冷凝、收取和首次扩筏；不显示按键教学、不写教程存档，鲨鱼、风暴、入水和上岛时主动让位；
...
- 六类原创结构使用分件装配与最多八个固定实例批次渲染（七个基础批次 + 一个临界断面批次）；共享边、格内、楼面占位和承重拓扑统一校验，板门可在世界中开合并参与玩家碰撞，拆除或鲨鱼咬毁筏格会阻止或级联清除失去支撑的结构；
- 六类结构拥有独立生命、完整/受损/临界三档材质与确定性松动变形；临界木件会缩短真实承力分件并露出 Image 2 high 雪松横截面，修复后断面批次归零；深潮鲨会从外沿选择暴露结构并优先追击弱点，撕咬、碎屑、空间断裂音、拓扑坍塌和当前 v18 写入共享同一事务；失去支撑的结构继续以复用原分件/材质的双块实体翻滚落海，触发泡沫、木屑、空间落水声后下沉回收，瞬态过程不污染存档；
...
- 最终 DCC 深潮鲨现有机器可读 GLB v1 合同与 CLI 验证器，锁定 BIN/accessor 边界、变换后实际坐标尺度、六类 PBR 材质映射、单 skin、13-48 关节拓扑、精确七段玩法动画集、1.6 MB/36k triangles/50k vertices 上限和无内嵌贴图策略；合同已就绪，最终模型与专用牙龈仍明确保持未完成；
...
- 候选发布链会从锁文件递归核对九个生产依赖的许可证，检查 113 个运行时资产与 45 个采用源图的来源记录，生成完整第三方许可文本、无 sourcemap 静态包和逐文件 SHA-256 清单，再从 `dist` 真实执行延迟加载与 `WEBGL_lose_context` 丢失/恢复冒烟测试；
- 六总线程序音频混音、统一母带峰值保护、按提示优先级自动让位的环境/音乐层、随完整相机姿态更新的 HRTF 落水/打捞定位、近场绳索受力与断裂层，以及水下低通、鲨鱼失力/浮尸/分段割取/下沉、震叉分段蓄能/就绪/失调/脉冲、生活/信号设备、锚帆、风雨雷声、种植、研究、礁区和生物声音；
- 标题、HUD、背包、制作、设置、能力提示和 Playwright 截图回归流程；
- 原创标题美术、木材、泡沫、深潮鲨主体真皮齿、口缘/鳃衬、侧眼与牙釉、编织纤维、AI 辅助海床、拼补帆布、培养土、耐火陶土、导航合金、信号层压板、磷光玻璃、共鸣青铜、电气陶瓷、潮缚索具、盐蚀工具钢、风暴撑紧固合金、风暴伤雪松横截面、岛岩、棕榈树皮/叶面、野生潮果、岸滩地表、盐蚀集热玻璃、蜡封帆布、盐封手套、三种鱼皮、生/熟/焦鱼肉、远洋鱼眼、耐热折铁、盐蚀聚合物、盐冠活/枯叶与潮果、盐翼体羽/飞羽/角质/虹膜 PBR 与飑云天空材质，以及对应的 normal/roughness 通道；风化雪松也已补齐 normal/roughness。

当前仍不是完整游戏。M6 钓鱼、烹饪/净水、天气农业、盐翼鸟害、M8 分阶段研究与远海目的地，以及 M9 三档存档/备份恢复、无障碍输入、情境化引导、候选发布链和七批历史材质整改的代码与自动视觉闭环已经完成；无说明玩家验收、目标真实 GPU 鼠标/双画质/20 分钟长稳门禁、项目自身发布许可选择、最终 DCC 鲨齿/蒙皮资产、更多深水生态资源和潜水装备仍按 [项目追踪](PROJECT_TRACKER.md) 继续开发。
...
M9 水下材质门禁复现：`CAPTURE_ONLY=underwater CAPTURE_FAST=1 npm run capture` 验证七套 PBR 的 21 个语义槽、七个 atlas 区域、`29/32` 纹理预算与真实 WebGL framebuffer；`CAPTURE_ONLY=underwater-interaction CAPTURE_FAST=1 npm run capture` 复验真实海草收割。详见 [M9 水下材质验收记录](docs/M9_UNDERWATER_MATERIAL_ACCEPTANCE.md)。
