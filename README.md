...
原创桌面网页 3D 海上生存游戏。当前版本为 `0.13.0` 高质量纵向切片，不以基础 Demo 为完成标准。
...
当前仍不是完整游戏。背包拆分/拖放、连续制作、更多深水生态资源、潜水装备、天气农业、其余工具耐久、大型信号目的地和最终蒙皮资产仍按 [项目追踪](PROJECT_TRACKER.md) 继续开发。
...
截图脚本默认连接 `http://127.0.0.1:4173`，支持 `DRIFTWAKE_URL`、`CHROMIUM_PATH`、`CAPTURE_WIDTH`、`CAPTURE_HEIGHT`、`CAPTURE_QUALITY` 和 `CAPTURE_ONLY`。目标包括 `title`、`pause`、`game`、`hook`、`salvage`、`failure`、`pack`、`crafting`、`devices`、`advanced`、`signal`、种植/研究/岛屿/水下/导航各主流程、`underwater-narrow`、`narrow`、`settings` 和 `mobile`。`salvage` 会实际近拾海面掉落、触发断钩、制作替代钩并检查手持/抛出模型唯一所有权；`failure` 会验证模拟暂停、一次性右舷落物、筏面恢复、数值复位和掉落保留；`pause` 会模拟 Pointer Lock 拒绝。3D 截图使用分布式 WebGL 像素门禁，拒绝黑屏、白屏、HUD 相交和丢失的上下文。
